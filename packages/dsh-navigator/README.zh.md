...
当前目录包含配置 schema、结论类型和可装载的插件入口（`name`、`inject`、`Config`、`apply`）：等待模式已能到点发起复核、把收场结算成一条记录，并按会话 id 读回与回放。三类结论对主会话的干预（建议注入、停止说明）与并行模式已落地，等待期作废亦已落地；任务结束 / 任务取消 / 重载卸载时的在途复核处置仍在实现中；本包不会修改 DeepSeek Harness 源码。

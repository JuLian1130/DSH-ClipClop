...
当前目录包含配置 schema、结论类型和可装载的插件入口（`name`、`inject`、`Config`、`apply`）：等待模式已能到点发起复核、把收场结算成一条记录，并按会话 id 读回与回放。三类结论对主会话的干预（建议注入、停止说明）与并行模式已落地，取消路径仍在实现中；本包不会修改 DeepSeek Harness 源码。

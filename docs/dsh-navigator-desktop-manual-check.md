...
**收窄退路**（设计文档已写死）：若 Desktop 这一路在本机确认走不通，就把联调范围收窄到 **CLI / Web / SDK**（这三条在本仓已有可重跑的自动化用例：`prebuilt-artifact.spec.ts`、`web-entry.spec.ts`、`sdk-entry.spec.ts`），并在记录里写清「Desktop 路径失败的**具体一步**与报错或现象原文」。
...
- [ ] 结论一句话模板（按结局挑一栏，证据只填这次真有的）：
      - 通过：`G3：Desktop <版本>（dsh <版本>，pnpm <版本>）按本清单联调（第 1–8 步），通过；证据：profile 里的安装副本 <路径>（第 4 步验过：预构建产物、装时未跑构建脚本——G3「安装时不发 prepare」那半）；复核记录 <路径>（插件真的被装载并激活——G3「解析到 profile 内」那半）；第 7 步的展开行——那一行在、展开含正文、切走再切回仍在 <截图/描述>（开头第 ② 条要的就是它；重载那一格只在第 7 步读，所以这一项一律来自 Part B）；未收窄。`
      - 失败：`G3：Desktop <版本>（dsh <版本>，pnpm <版本>）按本清单联调（第 1–8 步），失败在第 <N> 步：<报错或现象原文>；证据：这次真有的那些（安装副本 <路径> / profile 状态 / 复核记录 <路径> / 会话事件 / 界面截图）；<未收窄 | 已收窄到 CLI/Web/SDK，原因 …>。`

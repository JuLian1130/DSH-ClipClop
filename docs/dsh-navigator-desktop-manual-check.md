...
- [ ] `node_modules/@dsh-clipclop/dsh-navigator` 还在——第 4 步装进去的那份副本没有被启动过程删掉。（`ls -d` 只证这个路径在、不证身份；它是否真的被装载解析到，见第 6 步的激活与复核记录。）
...
- [ ] 结论一句话模板（按结局挑一栏，证据只填这次真有的）：
      - 通过 / 失败：`G3：Desktop <版本>（dsh <版本>，pnpm <版本>）上按本清单链接 tarball 产物并启用 bundle，<通过 | 失败>；证据：profile 里的安装副本 <路径>、复核记录 <路径>、对话里的折叠行 <截图/描述>；<未收窄 | 已收窄到 CLI/Web/SDK，原因 …>。`
      - 未验（第 0 步就停下）：`G3：未验——Desktop 的 dsh 是 <版本>，与承诺的 0.1.6-alpha.1 不符，在第 0 步停止：未装产物、未启用 bundle、无复核记录；证据：应用「关于 / 版本」读到的版本 <截图/描述>，日期 <日期>。`

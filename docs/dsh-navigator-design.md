...
- **等待复核期间到达的真实用户消息走的是另一条路径，必须单独处理。** 用户消息经 `followup` 进入「下一轮」（`packages/core/agent-loop/src/agent.ts:137-139`）或经 `steer` 进入「下一步」（`agent.ts:141-143`），两条路径都不在本步被 claim 的那一批里，所以上面那条检查看不到它们；而 `cancel` 默认清空整个待处理队列（`agent.ts:149-155`），一旦在此时停止，用户刚发的话就没了。因此等待期间要监听 `agent/inbox/inserted`（两条路径都覆盖），出现 `source.kind === 'user'` 的消息就中止复核、不作任何干预。（并行模式用的是同一个事件，但目的是改写已入队建议的过期标注，见上面「建议注入」。）**两支信号的规格前提不同，verdict 过滤只加在 claim 检查那一支**：在途到达那一支按规格「等待复核期间到达的真实用户消息一律让本次复核作废」，任何结论都改落一条状态「取消」、原因「作废」的记录；而 claim 检查的前提是「注入建议、追加停止说明或停止 turn 之前」，只在 `adjust` / `stop` 上作废——`continue` 本来就不注入也不停止、结论已解析成功，照旧落完成态。规格的记录表按状态取值（完成 = 拿到结论并解析成功），04 的完成态三态格建在这条上：等待模式的触发点那次 pre-step 正常会 claim 到用户自己那条消息，claim 检查若不过滤 `continue`，普通复核也会被记成取消。

...
**那道检查只挂在会产生干预动作的那一支**（等待 × `failurePolicy: stop`，即追加停止说明、停止 turn 之前），与「注入与停止机制」里「`verdict` 过滤只加在应用点检查那一支」同源：它是干预动作的前置检查，不产生干预动作的格不做。票据 11 的**释放 × `stop`** 是同一格——它同样追加停止说明、停止 turn，所以那道检查在释放路径上照跑；命中就作废（不追加说明、不停止），释放回调已经落下的那条 `plugin-disposed` 取消记录就是这一格的记录，不另写第二条。做成「所有失败路径都跑」会让 04 的失败列（同样是失败输入，触发点那次 pre-step 照旧 claim 到真实用户消息）被读成取消态、整列变红。落点改动不改变 04 失败列的读数——那一列按真实收场读记录，等待 × 默认策略仍恰一条。
...
**读写入口与 writer 的关闭次序**：记录在插件自有存储域里，外部行为观察不到，所以本包导出两个函数——写入入口（接受完整参数、落一条记录、用唯一的存储域）与读回入口（按会话 id 返回该会话的记录列表，已按声明 schema 解析、按触发步骤升序）。两者都是**随包发布的对外表面**，不只是测试面。运行时写入不得依赖模块级可变量：`apply` 内部持有的 writer 才是运行时唯一入口，它绑到本实例的域与自己的 disposer。次序不变式两条：①writer 承诺「记录已落盘」时本实例的域仍开着；②fiber 释放（`internal/update` 重启、卸载、重挂载）时，取消结算要么在域关闭之前落盘，要么按「无法落盘」处理，绝不写进一个已关闭或已换代的句柄。**释放路径（票据 11）按这两条落地**：插件在加载路径上注册一个**晚于** `openReviewStore` 的 fiber 释放 effect，卸载时它先于关域那个 disposer 跑，遍历在途登记、把每条登记里的记录基底**同步入队**。这条时序的两半都是源码事实而不是约定：Cordis 卸载一条 fiber 时 `_unload()` 把 `_disposables.clear()`（已逆序）的结果交给 `Promise.all`，而每个 disposer 在 `runDisposable` 之前只隔一次 `await Promise.resolve()`，所以**启动次序＝注册的逆序**（`@deepseek-ai/cordis` 的 `fiber.ts` 的 `_unload` 与 `effect`）；`KvTable.put` 在调用那一刻就把任务挂上域的写链，而 `Domain.close()` 只是置 `disposing` 之后再 `await chain`（排空已有写链、拒收新写入），所以「先入队、后关域」不受两个 disposer 之间异步竞速的影响（`@deepseek-ai/dsh-storage-domain` 的 `enqueue` 与 `runClose`）。写入失败按不变式②的后半句处理。**实测分界**：本节只有源码核对；一次性的同形假域探针（两个 effect + 假写链，跑 5 次同序，文件已删）与票据 11 的释放用例（真实存储栈上重跑；关掉那个释放 effect 时它们一起红）把它跑成运行时保证。释放落在等待步的 `await next()` 期间时，基底只差快照那一半，落下的仍是一条取消记录、只是 `messageIds` 缺省——记录表本就有「取到快照之前就结束则没有」这一格。

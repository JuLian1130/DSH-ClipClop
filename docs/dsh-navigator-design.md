...
- **多步驱动的机制**（02c 的集成夹具建在这上面；三处「形状即决定」的形状留在票面 02c 第 1 条，本节只留机制与出处）：turn 只在这三处收尾——assistant 消息不含 `tool-call`（`packages/core/agent-loop/src/agent.ts:488`）、某个工具的成功结果带 `concludesTurn`（同文件 `:493` 的 `concluded`，由 `ToolRunContext.concludeTurn()` 置位，`packages/core/tools/src/index.ts:417,1401`）、该步以 max-tokens 收尾（同文件 `:486`）。所以「一步一次 tool call」能把 turn 撑住；夹具注册的那个工具**不调** `concludeTurn()`，该步因此成功收尾而 turn 继续。本包解析不到 `@deepseek-ai/dsh-tools`（只在 `.pnpm` 里经 testkit 传递），工具定义因此手写：`register` **不校验** `parameters`，而 `output.schema` 走 `packages/core/tools/src/json-schema.ts` 的 `assertSupportedJsonSchema`（未知关键字与非对象 `items` 抛 `JsonSchemaError`，无 `type` 的注解式 schema 被接受）。适配器侧要按 `block-start` → `tool-call-delta` → `block-end` 三段把 `id` / `name` / `arguments` 发全，否则 loop 取不到完整调用。**步边界闸门的落位与理由写在夹具 `ctx.tools.register(...)` 处**（暂停必须落在 step N 的 `assistant/message` 之后、step N+1 的 pre-step 之前，且暂停期间重载 / 改配置后要用当前实例续跑），本节与票面都不复述。
- **请求归属到会话的机制**（02c 第 3 条）：loop 自己建的请求带 `sessionId`，插件自建的复核请求**不带**（`src/index.ts` 那次 `ctx.llm.stream({ … })` 没有这个字段），所以夹具按 agent 的 initiator 归属——loop 用 `withInitiator(this, () => this.kick())`（`packages/core/agent-loop/src/agent.ts:208`）把整段 driver 链标上发起者，`currentInitiator()` 读这条异步链上的 agent（`packages/core/agent/src/index.ts:292`，ALS 实现；`packages/core/agent-loop/src/tool-calls.ts:68,131` 也用它取当前 agent）。它按异步链继承、**没有时间上界**：链上后到的请求仍归这条会话。夹具**不**自己开「当前那一个窗口」：两条会话的 pre-step 交叠时窗口互相覆盖，主会话的复核请求会整批记到子会话名下（G15 的反证）。

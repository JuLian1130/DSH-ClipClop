...
- **应用任何结论前，先检查尚未被任何一步处理的消息。** 探针实测：在 `agent/pre-step` 阶段停止（`cancel` 和 `reject` 都一样），本步被 claim 的消息**不会**写入会话历史——claim 已经把消息取走，而 step 从未打开。如果用户消息正好落在这一批里，它会被静默丢弃。因此「本步被 claim 的批次 ∪ `nextStep` / `nextTurn` 待处理队列」里存在真实用户消息（`source.kind === 'user'`）时，准备干预的结论（`adjust` / `stop`）即作废：不注入、不停止，让该步正常继续；`continue` 不产生干预，照旧落完成态（队列那一半为什么不能省，见下条）。这落实了规格里「等待复核期间的真实用户消息让复核作废」这条。**任务取消与任务结束都从另一条路径来，应用点检查看不到它们。** 任务取消会先清空整个待处理队列再 abort（`agent.ts:149-155`），不产生新的 claim；此时 pre-step payload 的 `signal` 会 abort（`agent.ts:244` 取 `phase.abort.signal`，`:251` 传进 payload），可以从它察觉。**但任务结束（turn 正常收尾、会话转入空闲）不会 abort 这个 signal**：`kick` 的 finally 把 phase 置为 idle 且不 abort（`agent.ts:231-238`），而 `cancel` 只 abort 非 idle 的 phase（`:149-155`）。所以照「只监听 signal」实现，并行复核会在会话已经空闲之后照常跑完并投递建议，「任务结束一律取消在途复核」静默失效，规格里「任务结束」这种取消原因也永远取不到。任务结束的察觉信号是 `agent.whenIdle()`（`packages/core/agent/src/runtime-types.ts:191`）：它的语义是「整个 agent 的活动到达静止，并跟随此前启动的替代工作」，因此不会把「idle 后同一 tick 又被新输入唤醒」误判成任务结束。并行复核与它赛跑——`whenIdle()` 兑现时按 `signal.aborted` 决定写哪种取消原因：已 abort 就是「任务取消」，否则是「任务结束」。**不能用「谁先到」判定**：任务取消总是先 abort `signal`，但复核自己的请求什么时刻收场由传输层决定，可能早于也可能晚于 `whenIdle()`（G10 实测：abort 后 0ms 结算时复核先到、30ms 结算时 `whenIdle()` 先到，而那时 `signal.aborted` 已经是 true），按到达顺序会把它误记成「任务结束」。
- **等待复核期间到达的真实用户消息走的是另一条路径，必须单独处理。** 用户消息经 `followup` 进入「下一轮」（`packages/core/agent-loop/src/agent.ts:137-139`）或经 `steer` 进入「下一步」（`agent.ts:141-143`），两条路径都不在**送出那一刻那一步**被 claim 的那一批里（那一步的 claim 早于这条消息入队），所以上面那条检查看不到它们。它们的作废走两条信号：唯一的 `agent/inbox/inserted` 监听器（在途登记从触发点决定起，覆盖 `next()` 与复核请求），或应用点检查**尚未被任何一步处理的消息**——本步被 claim 的批次加上 `nextStep` / `nextTurn` 待处理队列。**队列这一半不能省**：claim 早于 waterfall 派发（中间还隔着 `systemPrompt.assemble` 与排在本插件之前的 handler 的序言），消息落在在途登记之前时既进不了 claim 批次、也没有在途对象可置位，只剩队列上看得见。而 `cancel` 默认清空整个待处理队列（`agent.ts:149-155`），一旦在此时停止，用户刚发的话就没了。因此等待期间要监听 `agent/inbox/inserted`（两条路径都覆盖），出现 `source.kind === 'user'` 的消息就中止复核、不作任何干预。（并行模式用的是同一个事件，但目的是改写已入队建议的过期标注，见上面「建议注入」。）**verdict 过滤只加在应用点检查那一支**：在途到达那一支按规格「等待复核期间到达的真实用户消息一律让本次复核作废」，任何结论都改落一条状态「取消」、原因「作废」的记录；而应用点检查那一支只在 `adjust` / `stop` 上作废——`continue` 本来就不注入也不停止、结论已解析成功，照旧落完成态。规格三处（「一律」、测试决策里「这种情况下写出的记录状态是「取消」」、「状态怎么取」）都没给结论加限定，所以这条过滤是本实现的取舍而非规格的读法：等待模式的触发点那次 pre-step 正常会 claim 到用户自己那条消息，`continue` 不过滤则普通复核也会被记成取消，04 的完成态三态格就会读到取消态而变红。规格的记录表按状态取值（完成 = 拿到结论并解析成功）。

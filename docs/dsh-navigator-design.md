...
- **取消原因要按 `signal.reason` 的 `kind` 分开**：取消原因是 `AgentCancelCause`（`packages/core/session/src/types.ts`）——`{ kind: 'user' | 'parent' | 'hook' | 'disposed' }`。插件释放与用户取消走**同一条 abort**（`cancel` 对非 idle 的 phase 只做 `phase.abort.abort(cause)`），差别只在 cause：释放的调用点是 `packages/core/agent-loop/src/index.ts` 的 `machine.cancel({ kind: 'disposed' })`（agent 级 dispose 则是 `prepared.agent.cancel({ kind: 'disposed' }, { keepInbox: true })`）。因此「任务结束 / 任务取消」那条路径的判据要在 `signal.aborted` 之上再看 `signal.reason`，把 `disposed` 留给插件重载 / 卸载那条路径——两票共用同一个「中止在途复核 + 落取消记录」入口，只有这个 `kind` 能把两者分开（票据 10 与 11 的分工见各自票面）。
